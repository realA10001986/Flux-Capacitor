...
//
// Adapted by Thomas Winischhofer (A10001986) in 2026
...
DSTATUS ff_sd_initialize(uint8_t pdrv)
{
    ...
    do {
        ...
        #ifdef TW_SD_DEBUG
        if(!sdWait(pdrv, 500)) {
            Serial.println("sdWait fail ignored, card initialize continues");
        ...
        token = sdCommand(pdrv, GO_IDLE_STATE, 0, NULL);
        sdDeselectCard(pdrv);
        ...
        #ifdef TW_SD_DEBUG
        Serial.printf("Attempt %d: GO_IDLE_STATE failed. Token %x\n", resp + 1, token);
        #endif
    ...
    // Try to identify SDHC/SDXC cards using SEND_IF_COND (CMD8) command
    if (sdTransaction(pdrv, SEND_IF_COND, 0x1AA, &resp) == 1) {
        ...
        // Read Operating Conditions Register to check card capabilities (CMD58)
        if (sdTransaction(pdrv, READ_OCR, 0, &resp) != 1 || !(resp & (1 << 20))) {
            #ifdef TW_SD_DEBUG
            Serial.printf("READ_OCR (1) failed: %X\n", resp);
            #endif
            goto unknown_card;
        }
        ...
        if (!sdTransaction(pdrv, READ_OCR, 0, &resp)) {
            ...
            #ifdef TW_SD_DEBUG
            Serial.printf("READ_OCR (2) failed: %X\n", resp);
            #endif
            goto unknown_card;
        }
    } else {
        ...
        if (sdTransaction(pdrv, READ_OCR, 0, &resp) != 1 || !(resp & (1 << 20))) {
            #ifdef TW_SD_DEBUG
            Serial.printf("READ_OCR (3) failed: %X\n", resp);
            #endif
            goto unknown_card;
        }
